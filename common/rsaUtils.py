...
# Originally written by Tom McCoy
...
def make_concept3(samps, grp1, grp2, grp3, attr):
    ...
    for i in range(len(samps)):
        ...
        for j in range(len(samps)):
            word2 = samps[j]
            if ((word1 in attr or word1 in grp1) and (word2 in attr or word2 in grp1)) or (
                    (word1 in grp2) and (word2 in grp2)) or ((word1 in grp3) and (word2 in grp3)):
            ...
            if ((word1 in attr or word1 in grp2) and (word2 in attr or word2 in grp2)) or (
                    (word1 in grp1) and (word2 in grp1)) or ((word1 in grp3) and (word2 in grp3)):
            ...
            if ((word1 in attr or word1 in grp3) and (word2 in attr or word2 in grp3)) or (
                    (word1 in grp1) and (word2 in grp1)) or ((word1 in grp2) and (word2 in grp2)):
...
def get_RSA3(sheet_path, embedding_path, dims):
    ...
    for idx, name in enumerate(sheets.sheet_names):
        ...
        while len(samps) < 100:
            ...
            sample = list(np.random.choice(range(0, len(group1)), replace=False, size=4)) + list(
                np.random.choice(range(len(group1), len(group1) + len(group2)), replace=False, size=4)) + list(
                np.random.choice(range(len(group1) + len(group2), len(group1) + len(group2) + len(group3)),
                                 replace=False, size=4)) + list(np.random.choice(
                range(len(group1) + len(group2) + len(group3), len(group1) + len(group2) + len(group3) + len(concept)),
                size=4))
            ...
            grp1_attr_model, grp2_attr_model, grp3_attr_model = make_concept3(samp_sentences, group1, group2, group3,
                                                                              concept)
